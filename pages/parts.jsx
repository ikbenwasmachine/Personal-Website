...
export default function Home() {
  ...
  return (
    <>
      <div className="py-20">
        <p className="text-3xl text-white font-semibold text-center">De onderdelen</p>
        <p className="text-xl text-white/50 font-normal text-center mb-5">De onderdelen waar Sofius uit bestaat, staan ​​in dit veld.</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-3 w-full gap-4 items-center mt-2">
          {_parts ? (
            parts ? (
              parts.map((_, __) => (
                <a href={_.link} target="_blank" rel="noreferrer" key={__} className="bg-neutral-800/10 p-4 hover:bg-neutral-800/20 shadow-lg hover:shadow-xl transition-all duration-200 rounded-lg w-full" style={{border: '1px solid black'}}>
